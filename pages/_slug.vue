    ...
    <div class="row">
      <div class="col-md-2 col-xs-12">
        <div class="share">
          ...
          <ul>
            <li>
              <a href="#" @click.prevent="shareTuite">
                <svg
                  class="svgIcon-use"
                  width="29"
                  height="29"
            ...
            <li>
              <a href="#" @click.prevent="shareFeice">
                <svg
                  class="svgIcon-use"
                  width="29"
                  height="29"
      ...
      <div class="col-md-8 col-md-offset-2 col-xs-12">
        <div class="mainheading">
          <h1 v-if="artigo.title" class="posttitle">{{ artigo.title }}</h1>
        </div>
        ...
        <!-- Begin Tags -->
        <div v-if="artigo.tags" class="after-post-tags">
          <ul class="tags">
            <li v-for="(tag, index) in artigo.tags" :key="index" class="mr-2">
...
export default {
  async asyncData({ $content, params }) {
    const artigo = await $content(params.slug).fetch()
  ...
  computed: {
    meta() {
      return siteMeta({
        ...
        mainImage: `${this.artigo.image}/1200x630`,
      })
    },
  },
  methods: {
    shareTuite() {
      const referer = encodeURIComponent(document.location.origin)
      const refSrc = 'twsrc%5Etfw'
      const text = this.artigo.title
        ? encodeURIComponent(this.artigo.title)
        : ''
      const link = document.location.href
      const url = `https://twitter.com/intent/tweet?original_referer=${referer}&ref_src=${refSrc}&text=${text}&tw_p=tweetbutton&url=${link}`
      window.open(url)
    },
    shareFeice() {
      const referer = encodeURIComponent(document.location.origin)
      const url = `https://www.facebook.com/sharer/sharer.php?u=${referer}`
      window.open(url)
    },
  },
  head() {
    return {
      ...
      meta: [
        ...this.meta,
        {
          ...
          content: this.artigo.createdAt,
        },
        {
          property: 'article:modified_time',
          content: this.artigo.updatedAt,
        },
      ],
    }
  },
